...
export default {
  ...
  methods: {
    ...
    filterChange() {
      ...
      //判断分组
      visible = this.select.groupBy.every(it => {
        if (it.value === undefined || it.value === null || it.value === "")
          return true;
        const value = data[it.field];
        return it.value.includes(value);
      });

      if (visible === true) {
        visible = new RegExp(escapeRegexpString(this.query), "i").test(
          this.currentLabel
        );
      }

      this.visible = visible || this.created;
  ...
  created() {
    ...
    this.$on("groupChange", this.groupChange);
