...
export default {
  ...
  props: {
    ...
    multiple: Boolean,
    fakeMultiple: Boolean, //用于像预订单客户需求汇总啊，这种单选但是不要隐藏弹出框，可以连续选择
    multipleLimit: {
      type: Number,
      default: 0
    },
  ...
  watch: {
    ...
    group: {
      handler(val) {
        if (val && val.length > 0) {
          this.groupBy = val.map(it => ({ ...it, value: "" }));
        } else {
          this.groupBy = [];
        }
      },
  ...
  methods: {
    ...
    handleGroupChange(field, value) {
      if (field === undefined) {
        this.groupBy = this.group.map(it => ({ ...it, value: "" }));
      } else {
        const index = this.groupBy.findIndex(it => it.field === field);
        this.$set(this.groupBy[index], "value", value);
      }

      this.filteredOptionsCount = this.optionsCount;
      this.broadcast("ElOption", "groupChange");
    },
